...
- [x] `docker build -t naviqore-viewer .` succeeds from the repo root.
- [x] The built container serves the Vite app correctly: `docker run -p 8080:8080
naviqore-viewer` and opening `http://localhost:8080` returns the app with the correct
      title. (nginx listens on 8080; root and deep routes verified locally.)
- [x] Client-side routing works inside the container (navigating to a deep route and
      refreshing does not return 404). (`try_files $uri $uri/ /index.html` in nginx.conf.)
- [x] The runtime container process runs as a non-root user (verifiable via
      `docker inspect` or `whoami` inside the container). (`USER appuser`, uid 1001.)
- [x] The `Dockerfile` uses a fixed digest-pinned or version-pinned base image tag (e.g.
      `node:22-alpine`, `nginx:1.27-alpine`) — no `latest` base images.
- [x] `nginx.conf` sets `Cache-Control: max-age=31536000, immutable` for hashed asset
      paths (`/assets/*`) and `no-cache` for `index.html`.
- [x] `docker.yml` CI workflow runs on every PR to `main` targeting the Docker build
      validation step; failing build blocks the PR.
- [x] The release publish workflow pushes to `ghcr.io/<org>/<repo>:latest` and
      `ghcr.io/<org>/<repo>:<semver>` on release-please release events.
- [x] Image is labelled with OCI standard labels (`org.opencontainers.image.*`).
- [x] `npm run check` and `npm run test:run` still pass (Docker files do not affect
      TypeScript source).
...
- Date: 2026-03-09
- Outcome: All ACs satisfied. Multi-stage Dockerfile (`node:24-alpine` build +
  `nginx:1.27-alpine` runtime) with custom `nginx.conf` (SPA routing, gzip, cache
  headers). Non-root `appuser` runs nginx on port 8080. `docker-ci.yml` adds a
  quality gate before Docker build on PRs. `release-please.yml` extended with a
  `publish-docker` job that pushes `:latest` and `:<semver>` to ghcr.io on release.
  `.nvmrc` added to pin Node 24; CI workflows read from it. Build verified locally:
  `docker build` succeeds, root and deep SPA routes both return 200, process runs
  as `appuser`.
- Descoped ACs: none — all ten ACs satisfied.
